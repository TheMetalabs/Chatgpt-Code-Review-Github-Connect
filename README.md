...
| `ASHLAR_LOCAL_REVIEW_SKIP_REPOS` | (비어 있음) | 로컬 LLM 리뷰 레그를 **건너뛸 저장소** 목록(`owner/repo`, 쉼표·공백 구분, 대소문자 무시). 나열된 저장소는 채팅 리뷰어(ChatGPT/Grok)만으로 리뷰합니다 — 비FIFO인 로컬 대기열에서 특정 저장소 리뷰가 몇 시간씩 밀리는 것을 막는 운영 스위치. 채팅 리뷰어가 하나도 없으면 로컬을 빼지 않습니다(리뷰 누락 방지). 잡마다 읽고, 진행 중인 잡의 리뷰어는 바뀌지 않습니다 |
