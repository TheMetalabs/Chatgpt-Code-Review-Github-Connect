...
- **작성자 강제:** start 기록·마커·CONVERGED는 봇(App 로그인)만, 사람은 **편집되지 않은** 코멘트의 stop만(작성 시각).
  ...
  새 start 뒤에 재전송)은 STOPPED 마커 없이 기록 줄 `<!-- ashlar-loop-stop at= by= -->` 로 시작한다 — 세션 fold는 같은
  정지로 읽지만, 진행 중인 새 세션을 두고 «루프 정지» 종착 신호를 내지 않는다(§3).
...
STOPPED 마커(첫 줄)는 기록을 게시하기로 한 조회 시점에 **진행 중인 세션이 없을 때만** 나간다. 정지 기록 줄
