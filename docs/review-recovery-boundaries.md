# Review recovery boundaries (extension 1.1.17)

Follow-up to the PR #35 / #36 review, based on main `4c14d45`.
This changes the extension only. It does not include the separate JSON-parser
or original-response dashboard work and does not change the bridge API.
...
elapsed-time deadline. Existing short control-message limits, bounded tab
capacity, independent job heartbeats and job/provider/run separation remain.
No active tab is closed to make room.

Update the complete files in the SAME installed unpacked extension directory
to 1.1.17 and reload that extension. Do not remove/reinstall it, clear storage,
