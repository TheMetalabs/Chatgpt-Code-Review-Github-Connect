...
type Detail = {
    captures?: {id: string; provider: string; runId: string; responseId: string; sourceHash: string; totalChars: number; at: number; text?: string}[];
...
export function HistoryBrowser({ initialKind = "jobs" }: {
    ...
    {detail && access ? <article className="rounded border border-line bg-bg-elevated p-4 space-y-4">
      ...
      <ol className="space-y-3">{detail.steps.map(item => <li key={item.id} className="border-l-2 border-line pl-3"><div className={unlabelledStep(item) ? "text-sm text-warn" : "text-sm"} title={unlabelledStep(item) ? "The extension recorded this stage before it had a history label; history keeps only a hash of its name" : undefined}>{stepLabel(item)}</div><div className="font-mono text-xs text-fg-muted">Received: {time(item.at)} · {item.source} {item.provider || ""}{item.observedAt ? ` · observed: ${time(item.observedAt)}` : ""}<br />{item.runId ? `Run: ${item.runId} · ` : ""}{item.id}</div></li>)}</ol>
