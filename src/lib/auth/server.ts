...
import { ashlarAllowedHosts } from "../ashlar-env";
...
const tunnelHosts = ashlarAllowedHosts();
const tunnelOrigins = tunnelHosts.flatMap((host) => [`https://${host}`, `http://${host}`]);
const baseURL = explicitBaseURL ?? {
  // Include loopback hosts so dynamic baseURL resolves for local email/password
  // (not only the preview wildcard).
  allowedHosts: [...previewAllowedHosts, "localhost", "127.0.0.1", "[::1]", ...tunnelHosts],
...
const trustedOrigins: string[] = explicitBaseURL
  ? [explicitBaseURL, ...LOCAL_DEV_ORIGINS, ...tunnelOrigins]
  : [
