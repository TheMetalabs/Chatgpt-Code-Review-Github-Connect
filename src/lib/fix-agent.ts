...
export interface FixRoundResult {
  ...
  outcome: "applied" | "suggested" | "parse-failed" | "commit-failed";
  error?: string;
}
...
export function buildFixPrompt(input: {
  ...
  return [
    ...
    "4. Return the COMPLETE new content of each changed file (never a diff, never elisions",
    "   like '// ... rest unchanged'). Unsafe/absolute/`..` paths are rejected.",
    "",
    `Changed files in scope: ${input.changedPaths.join(", ") || "(none listed)"}`,
    "",
    "Output schema (return exactly this shape, no prose outside the JSON):",
    '{ "summary": "<what you changed and why>", "files": [ { "path": "<repo-relative>", "content": "<full new file>" } ] }',
    "",
...
export async function runFixRound(
  ...
  opts: { prompt: string; mode: FixMode; branch: string; baseCommitSha: string; message: string },
