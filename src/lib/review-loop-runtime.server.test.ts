...
describe("CONVERGED is the posted outcome, not \"no findings\" (docs/local-verify-clean.md §1)", () => {
  ...
  const notClean: Array<[string, Partial<Job>, RegExp]> = [
    ["raw (a chat reply posted verbatim)", { reviewProviders: ["chatgpt"], rawReview: "P1 a.ts:1 CHAT-RAW", rawCauses: { chatgpt: "unparseable" } }, /posted verbatim: the reply was not parseable JSON\)/],
  ...
  for (const [name, patch, detail] of notClean) {
    it(`${name}: an active session gets one fixed handoff, never a silent stop`, async () => {
      ...
      // Only a recorded parse failure is handed off as one: never unread rows or an unknown cause.
      if (patch.rawReview && patch.rawCauses?.chatgpt !== "unparseable") assert.doesNotMatch(escalations(f.posted)[0], /not parseable|local repair/i);
