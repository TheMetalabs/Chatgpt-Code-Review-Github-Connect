...
function Reviews() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 md:px-8">
      ...
      <h1 className="mt-2 text-3xl font-medium tracking-tight">Posted thread</h1>
      <p className="mt-2 max-w-2xl text-sm leading-relaxed text-fg-muted">
        Poster talks to the Reviews API only. Same head SHA is dismissed before a replacement goes up. Models never see
        this screen.
      </p>

      {!active ? (
        <p className="mt-10 text-sm text-fg-muted">No reviews yet. Fire a sample PR from Operations.</p>
      ) : (
        <div className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1fr)_20rem]">
          <div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              {active.dismissed ? <Badge tone="muted">dismissed</Badge> : <Badge tone="ok">current</Badge>}
            </div>
          ...
          <aside className="space-y-4">
            {!active.dismissed ? (
              <div className="rounded-xl border border-line bg-bg-elevated p-4">
                <h3 className="text-sm font-medium">Mention the bot</h3>
              ...
              <div className="rounded-xl border border-line p-4 text-sm text-fg-muted">
                This review was dismissed. Mention the current thread, not an old SHA.
              </div>
            )}
